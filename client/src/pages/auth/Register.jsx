...
import { Mail, Lock, User, Loader2, Play, AlertCircle } from 'lucide-react';
...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    dispatch(clearError());
    const resultAction = await dispatch(registerUser({ username, email, password, displayName }));
    if (registerUser.fulfilled.match(resultAction)) {
      ...
      if (errorMessage.toLowerCase().includes('email already')) {
        toast('Redirecting to Sign In...', { icon: '🔑' });
        setTimeout(() => {
      ...
      <div className="max-w-md w-full bg-[#16161980] backdrop-blur-xl border border-[#27272acc] rounded-2xl p-8 shadow-2xl z-10 animate-slideUp">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-zinc-400 uppercase px-1">Display Name (Optional)</label>
          ...
          {/* Username Input */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-zinc-400 uppercase px-1">Username *</label>
            <div className="relative">
              <input
                type="text"
                required
        ...
        <p className="text-xs text-zinc-400 text-center mt-8 font-semibold">
          ...
          <Link to="/auth/login" className="text-accent hover:text-accent">
            Sign in
          </Link>
        </p>
      </div>
